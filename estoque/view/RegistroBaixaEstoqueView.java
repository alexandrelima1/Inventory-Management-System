...
public class RegistroBaixaEstoqueView extends JPanel {
    ...
    public RegistroBaixaEstoqueView(EstoqueService estoqueService) {
        ...
        // Texto estilizado para o título "Registrar Baixa"
        JLabel tituloLabel = new JLabel("Registrar Baixa");
        ...
        JButton registrarBaixaButton = new JButton("Registrar Baixa");
        registrarBaixaButton.setPreferredSize(new Dimension(150, 40));
        registrarBaixaButton.addActionListener(e -> registrarBaixa());
        ...
        backgroundPanel.add(registrarBaixaButton, gbc);
    ...
    private void registrarBaixa() {
        if (!validarCampos()) {
            return;  // Se houver erros de validação, interromper o registro
        }

        try {
            String produtoSelecionado = (String) comboProduto.getSelectedItem();
            String descricaoProduto = descricaoField.getText();  
            ...
            String descricaoBaixa = descricaoField.getText();  
            int fga = Integer.parseInt(fgaField.getText());

            // Passar a descrição do produto ao registrar a baixa
            estoqueService.registrarBaixa(produtoSelecionado, descricaoProduto, conexFuncionarioId, fieldFuncionarioId, quantidade, descricaoBaixa, fga);
            JOptionPane.showMessageDialog(this, "Baixa registrada com sucesso!");

            limparCampos(); // Limpa os campos após o registro

        } catch (SQLException | NumberFormatException e) {
            JOptionPane.showMessageDialog(this, "Erro ao registrar baixa: " + e.getMessage(), "Erro", JOptionPane.ERROR_MESSAGE);
        }
    }
